...
{ 
	...
	omni_mutex_lock l(m_desktop->m_screenCapture_lock);
...
#ifdef _DEBUG
	char			szText[256];
	sprintf(szText, "####################################Handle_Ringbuffer %i %i \n",ringbuffer->counter, g_Oldcounter);
...
vncDesktopThread::run_undetached(void *arg)
{
	...
	while (TRUE)
	{
		if (!PeekMessage(&msg, NULL, NULL, NULL, PM_REMOVE) || idle_update)
		{
			#ifdef _DEBUG
					char			szText[256];
					sprintf(szText," vncdesktopthread:PeekMessage \n");
					OutputDebugString(szText);
			#endif
			idle_update=false;
			// MAX 30fps
			...
			if (m_desktop->Hookdll_Changed && !m_desktop->m_hookswitch)
			{
				...
				m_desktop->StartStophookdll(m_desktop->On_Off_hookdll,m_desktop->m_hwnd);
			...
			if (	m_desktop->m_displaychanged ||									//WM_DISPLAYCHANGE
					...
					)
					{
						...
						//*******************************************************
						{
							...
							//*******************************************************
							if (m_desktop->m_displaychanged || !vncService::InputDesktopSelected() || m_desktop->m_hookswitch || (monitor_changed && !m_desktop->m_screenCapture))
									{
...
//					vnclog.Print(LL_INTINFO, VNCLOG("5 %i %i %i %i \n"),m_desktop->m_Cliprect.br.x,m_desktop->m_Cliprect.br.y,m_desktop->m_Cliprect.tl.x,m_desktop->m_Cliprect.tl.y);
			...
			#ifdef _DEBUG
					//char			szText[256];
					sprintf(szText,"vncdesktopthread: UpdateWanted ? \n");
					OutputDebugString(szText);
			#endif
			if (m_desktop->m_server->UpdateWanted())
			{
				#ifdef _DEBUG
					char			szText[256];
					sprintf(szText,"vncdesktopthread: UpdateWanted yes \n");
					OutputDebugString(szText);
				#endif
				//TEST4
				...
				//************* Polling ---- no driver
				//****************************************************************************
				if (!m_desktop->m_hookdriver || m_desktop->no_default_desktop)
				{
					#ifdef _DEBUG
					char			szText[256];
					sprintf(szText,"vncdesktopthread: Polling \n");
					OutputDebugString(szText);
					#endif
					// POLL PROBLEM AREAS
...
//								(!m_desktop->RestartDriver)
				...
				#ifdef _DEBUG
//					char			szText[256];
					sprintf(szText,"vncdesktopthread: PROCESS THE MOUSE POINTER \n");
					OutputDebugString(szText);
					#endif
...
//									vnclog.Print(LL_INTINFO, VNCLOG("6 %i %i %i %i \n"),rect.br.x,rect.br.y,rect.tl.x,rect.tl.y);
...

				{
					#ifdef _DEBUG
					char			szText[256];
					sprintf(szText,"vncdesktopthread: omni_mutex_lock1 \n");
					OutputDebugString(szText);
					#endif
					...
					#ifdef _DEBUG
//					char			szText[256];
					sprintf(szText,"vncdesktopthread: omni_mutex_lock2 \n");
					OutputDebugString(szText);
					#endif
					bool s_moved = false;
					// CHECK FOR COPYRECTS
...
//														vnclog.Print(LL_INTINFO, VNCLOG("6 %i %i %i %i \n"),rect.br.x,rect.br.y,rect.tl.x,rect.tl.y);
					...
					#ifdef _DEBUG
//					char			szText[256];
					sprintf(szText,"vncdesktopthread: SCAN THE CHANGED REGION FOR ACTUAL CHANGES \n");
					OutputDebugString(szText);
					#endif
...
#ifdef _DEBUG
//					char			szText[256];
					if (changedrgn.is_empty())
					{
						sprintf(szText,"vncdesktopthread: No Update \n");
//						idle_update=true;
					}
					else sprintf(szText,"vncdesktopthread: Update \n");
					OutputDebugString(szText);
#endif
			...
			#ifdef _DEBUG
					//char			szText[256];
					sprintf(szText,"vncdesktopthread: UpdateWanted NO \n");
					OutputDebugString(szText);
			#endif
			...
			#ifdef _DEBUG
					//char			szText[256];
					sprintf(szText,"vncdesktopthread: UpdateWanted NO2 \n");
					OutputDebugString(szText);
			#endif
		}//peek message
...
//			vnclog.Print(LL_INTERR, VNCLOG("REct %i %i %i %i  \n"),rect.tl.x,rect.br.x,rect.tl.y,rect.br.y);
		...
		#ifdef _DEBUG
					char			szText[256];
					sprintf(szText,"vncdesktopthread: PeekMessage Loop \n");
					OutputDebugString(szText);
		#endif
	}//while
