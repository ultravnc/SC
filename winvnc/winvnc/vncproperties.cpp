...
vncProperties::Init(vncServer *server)
{
	...
	m_pref_UseNewDSMPlugin = g_Newplugin;
...
//	m_server->SetPassword(m_pref_passwd);
	...
	// DSM Plugin prefs
	m_server->EnableDSMPlugin(m_pref_UseDSMPlugin || m_pref_UseNewDSMPlugin);
	if (m_pref_UseDSMPlugin)
		m_server->SetDSMPluginName("MSRC4Plugin.dsm");
	if (m_pref_UseNewDSMPlugin)
		m_server->SetDSMPluginName("SecureVNCPlugin.dsm");
