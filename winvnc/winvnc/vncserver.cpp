...
//	vnclog.Print(LL_INTINFO, VNCLOG("Lock3\n"));
	...
	if (RemoveWallpaperEnabled())KillWallpaper();
...
#ifdef _DEBUG
					char			szText[256];
					sprintf(szText," vncServer::UpdateWanted Lock \n");
					OutputDebugString(szText);
#endif
	omni_mutex_lock l(m_clientsLock);
#ifdef _DEBUG
//					char			szText[256];
					sprintf(szText," vncServer::UpdateWanted \n");
					OutputDebugString(szText);
#endif
...
MatchStringToTemplate(const char *addr, UINT addrlen,
	...
	if (addrlen < filtlen)
		return 0;
	for (unsigned int x = 0; x < filtlen; x++) {
		if (addr[x] != filtstr[x])
			return 0;
	}
