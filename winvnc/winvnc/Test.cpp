  ...
  if (os.GetVersion(&osvi))
  {
    ...
    switch (osvi.EmulatedPlatform)
    {
      ...
      case COSVersion::WindowsNT:
      {
        ...
        {
			_tcscat(sText, _T("Windows Vista: Warning UAC windows need to be controled by local user, to view mouse during this action, select Let server deal with mouse (viewer option)"));
		}
		else if (os.IsWindowsWin7(&osvi))
        {
			_tcscat(sText, _T("Windows Win7: Warning UAC windows need to be controled by local user, to view mouse during this action, select Let server deal with mouse (viewer option)"));
		}
